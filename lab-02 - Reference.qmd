...
3. Populate its YML with a title, author, subtitle, output type and theme. For example:
...
```{r}
st_boundary <- us_states(resolution = 'low') %>%
...
ggplot() + 
  geom_sf(data = st_union(states), fill = "grey", lty = 2, size = 0.5) + 
  geom_sf(data = cities, aes(color = as.numeric(distance_to_us_border)), size = .5) + 
  geom_sf(data = largest_10, color = 'black', size = .8) + 
