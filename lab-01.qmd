...
```{r}
colo_join <- inner_join(pop_colo, co_dat_cum, by = 'fips')
colo_join_tidy <- colo_join %>%
  select(fips, STNAME, CTYNAME, POPESTIMATE2021, date, county, cases, deaths, new_cases, new_deaths)

colo_join_tidy <- colo_join_tidy %>%
...
```{r}
slice_max(colo_join_tidy, n = 5, order_by = casespc) %>%
  select(STNAME, CTYNAME, POPESTIMATE2021, date, cases, new_cases, casespc) %>%
  flextable() %>%
  set_caption("Top 5 Counties with Most Cumulative Cases per Capita")

slice_max(colo_join_tidy, n = 5, order_by = newcasespc) %>%
  select(STNAME, CTYNAME, POPESTIMATE2021, date, cases, new_cases, newcasespc) %>%
  flextable() %>%
  set_caption("Top 5 Counties with Most Cumulative Cases per Capita")
```
