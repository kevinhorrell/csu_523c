...
```{r}
slice_max(colo_join2021, n = 5, order_by = casespc) %>%
...
slice_max(colo_join2021, n = 5, order_by = newcasespc) %>%
...
slice_max(colo_join_14, n = 5, order_by = tot_new_cases_std) %>%
...
To do this, we need to join the COVID data with location information. I have staged the latitude and longitude of county centers [here](https://raw.githubusercontent.com/mikejohnson51/csu-ess-330/refs/heads/main/resources/county-centroids.csv). 
...
pop_state_season <- pop_state_join %>%
  ...
  summarise(seasonal_cases = sum(new_cases),
            ...
            population = case_when(year == 2020 ~ first(POPESTIMATE2020),
                                   ...
                                   TRUE ~ NA_real_)) %>%
  distinct() %>%
  mutate(log_seasonal_cases = log(seasonal_cases + 1),
         log_seasonal_deaths = log(seasonal_deaths + 1),
...
```{r}
cases_lm <- lm(log_seasonal_cases ~ log_seasonal_deaths*log_population + season, data = pop_state_season)
summary(cases_lm)
```
**Results**
r-squared = 0.8076
...
Since the p-value is so low, the overall model shows that the cases are dependent on a combination of deaths, season, and population. However digging into the data, population, the intersection of deaths and population, Winter and Summer seasons are not significantly affecting the number of cases. Overall though, the model is useful and could be used to predict cases based on deaths, season, and population.
...
```{r}
pred_deaths <- broom::augment(cases_lm, new_data = pop_state_seas)
