...
plotRGB(r_palo, r = 4, g = 3, b = 2, stretch = 'hist')
```
...
## _Step 4.2_
  ...
  - Check the dimensions of the extracted values with dim
    ...
    *the data was extracted in columns and rows, a data frame and it keeps the same properties as the original raster*
...
```{r}
head(v)
palo_kmeans <- kmeans(v, centers = 12)
...
```{r}
clus_rast <- r_palo_indices$ndvi
values(clus_rast) <- palo_kmeans$cluster
